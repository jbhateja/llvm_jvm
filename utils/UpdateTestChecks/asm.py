...
ASM_FUNCTION_JVM_RE = re.compile(
    r'^.method\s+public\s+static\s+(?P<func>[^\(]*)[^\n]*\n'
    r'^\s*.limit[^\n]*\n'
    r'^\s*.limit[^\n]*\n'
    r'^\s*(?P<body>.*?)\n'
    r'^.end\s+method',
...
def build_function_body_dictionary_for_triple(args, raw_tool_output, triple, prefixes, func_dict):
  target_handlers = {
      ...
      'jvm32': (scrub_asm_jvm, ASM_FUNCTION_JVM_RE),
      'jvm64': (scrub_asm_jvm, ASM_FUNCTION_JVM_RE),
  }
  handlers = None
...
###### Emit label at function/method start
def check_emitting_label_at_func_start(triple):
    if (triple.startswith("jvm")):
        return False
    else:
        return True
