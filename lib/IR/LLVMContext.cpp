...
void LLVMContext::emitError(const Twine &ErrorStr, DiagnosticSeverity Severity) 
{
  diagnose(DiagnosticInfoInlineAsm(ErrorStr, Severity));
}
...
StringMap<StructType *> & LLVMContext::getNamedStructTypes() {
  return pImpl->NamedStructTypes;
}
