...
static cl::opt<bool> EnableFunctionVirtulization("enable-cs-codesplit",
    cl::Hidden,
    cl::desc("Checks and generates JVM bytecode for functions which "
             "are valid translation candidates"));

static cl::opt<bool> JvmCandidateChecker(
    "jvm-candidate-checker",
    cl::desc("Verifies function IR is valid for translation."),
    cl::init(false));
...
void TargetPassConfig::addIRPasses() {
  // Checks if function could be translated to JVM byte code.
  if (EnableFunctionVirtulization || JvmCandidateChecker) 
    addPass(createJVMTranslationPass());
  switch (UseCFLAA) {
