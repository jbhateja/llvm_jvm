...
void SelectionDAGBuilder::visitRet(const ReturnInst &I) {
  ...
  //TODO:Committing pending loads before processing return, for JVM.
...
void SelectionDAGBuilder::visitGetElementPtr(const User &I) {
  SDLoc dl = getCurSDLoc();
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  if (TLI.generateCompositeMacros() &&
      isa<CompositeType>(cast<PointerType>(
         I.getOperand(0)->getType())->getElementType())) { 
    SDValue N = TLI.createCompositeMacros(this, I, dl);
    setValue(&I, N);
    return;
  }
...
void SelectionDAGBuilder::visitAlloca(const AllocaInst &I) {
  ...
  if (TLI.generateAllocationMacros()) { 
    SDValue N = TLI.createAllocationMacros(this, I, dl);
    setValue(&I, N);
    return;
  }
...
void SelectionDAGBuilder::visitLoad(const LoadInst &I) {
  ...
  bool IsAbsorptionCandidate = TLI.checkIfAbsorptionCandidate(Ptr);
  Type *Ty = I.getType();
...
void SelectionDAGISel::LowerArguments(const Function &F) {
  ...
  LLVM_DEBUG({
    for (unsigned i = 0, e = Ins.size(); i != e; ++i) {
      ...
      if (!TLI->skipArgumentTypeValidation(Ins[i].Flags.isByVal()))
