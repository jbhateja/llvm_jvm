...
static int CountOperands(SDNode * Node)  {
  unsigned NumOpr = Node->getNumOperands();
  while (NumOpr && Node->getOperand(NumOpr - 1).getValueType() == MVT::Glue)
    --NumOpr;
  if (NumOpr && Node->getOperand(NumOpr - 1).getValueType() == MVT::Other)
    --NumOpr; // Ignore chain if it exists.
  return NumOpr;
}

static bool IsConnectedToChain(SDNode * Node) {
...
void ScheduleDAGSDNodes::BuildSchedUnits() {
  ...
  while (!Worklist.empty()) {
    ...
    // Bind all its glued operands to same scheduling unit.
    if (NI->getNodeId() != -1) {
      if (NI->getFlags().hasGlueOperands())
        for (int i = 0; i < CountOperands(NI); i++) {
          SDNode * Oper = NI->getOperand(i).getNode();
          if (!IsConnectedToChain(Oper))
            Oper->setNodeId(NI->getNodeId());
        }
      continue;
    }
    ...
    N = NI;
    if (N->getFlags().hasGlueOperands()) {
      for (int i = 0; i < CountOperands(N); i++) {
        SDNode * Oper = N->getOperand(i).getNode();
        if (!IsConnectedToChain(Oper))
          Oper->setNodeId(NodeSUnit->NodeNum);
      }
    }
...
void ScheduleDAGSDNodes::AddSchedEdges() {
  ...
  for (unsigned su = 0, e = SUnits.size(); su != e; ++su) {
    ...
    SmallVector<SDNode*, 4> Nodelist;
    for (SDNode *N = SU->getNode(); N; N = N->getGluedNode())
       Nodelist.push_back(N);
    
    if (SU->getNode()->getFlags().hasGlueOperands()) {
      SDNode * N = SU->getNode();
      for (int i = 0; i < CountOperands(N); i++)
         Nodelist.push_back(N->getOperand(i).getNode());
    }
    
    // Find all predecessors and successors of the group.
    while(Nodelist.size()) {
      SDNode * N = Nodelist.back();
      Nodelist.pop_back();
...
MachineBasicBlock *ScheduleDAGSDNodes::EmitSchedule(MachineBasicBlock::iterator &InsertPos) {
  ...
  for (unsigned i = 0, e = Sequence.size(); i != e; i++) {
    ...
    if (SU->getNode()->getFlags().hasGlueOperands()) {
      for(int I = CountOperands(SU->getNode())- 1, E = 0; I >= E; I--) {
        SDValue Oper = SU->getNode()->getOperand(I);
        // Leaf node, e.g. a TargetImmediate.
        if (!isPassiveNode(Oper.getNode()) && !IsConnectedToChain(Oper.getNode())) 
          GluedNodes.push_back(Oper.getNode());
      }
    }
