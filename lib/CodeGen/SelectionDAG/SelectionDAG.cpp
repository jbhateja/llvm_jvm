...
static void AddNodeIDNode(const TargetLowering * TLI, FoldingSetNodeID &ID, unsigned short OpC,
                          SDVTList VTList, ArrayRef<SDValue> OpList) {
  // Add an extra integer to key for nodes which need not be shared.
  // Here its preventing the sharing of morphed nodes, which would 
  // otherwise get CSEed.
  if (TLI && TLI->doNotCSE(OpC))
    ID.AddInteger(const_cast<TargetLowering*>(TLI)->getNodeReplicatorCounter(OpC));
  AddNodeIDOpcode(ID, OpC);
  AddNodeIDValueTypes(ID, VTList);
  AddNodeIDOperands(ID, OpList);
}

static void AddNodeIDNode(const TargetLowering * TLI, SDNode * N, FoldingSetNodeID &ID, unsigned short OpC,
                          SDVTList VTList, ArrayRef<SDValue> OpList) {
  // Add an extra integer to key for nodes which need not be shared.
  if (TLI && TLI->doNotCSE(N->getOpcode()))
    ID.AddInteger(const_cast<TargetLowering*>(TLI)->getNodeReplicatorCounter(N->getOpcode(),N));
  AddNodeIDOpcode(ID, OpC);
  AddNodeIDValueTypes(ID, VTList);
  AddNodeIDOperands(ID, OpList);
}
...
/// data.
static void AddNodeIDNode(const TargetLowering * TLI, FoldingSetNodeID &ID, const SDNode *N) {
...
static bool doNotCSE(SDNode *N, const TargetLowering * TLI) {
  if (TLI->doNotCSE(N->getOpcode()))
    return true;
...
#endif
  if (TLI && TLI->doNotCSE(N->getOpcode()))
    const_cast<TargetLowering*>(TLI)->addNodeToReplicationMap(N);
}
...
bool SelectionDAG::RemoveNodeFromCSEMaps(SDNode *N) {
  ...
  if (!Erased && N->getValueType(N->getNumValues()-1) != MVT::Glue &&
      !N->isMachineOpcode() && !doNotCSE(N,TLI)) {
    N->dump(this);
    dbgs() << "\n";
...
SDNode *SelectionDAG::FindModifiedNodeSlot(SDNode *N, SDValue Op,
  ...
  SDValue Ops[] = { Op };
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, N, ID, N->getOpcode(), N->getVTList(), Ops);
  AddNodeIDCustom(ID, N);
...
void SelectionDAG::clear() {
  ...
  Root = getEntryNode();
  DbgInfo->clear();
  const_cast<TargetLowering*>(TLI)->clearAuxiliaryData();
}
...
SDValue SelectionDAG::getConstant(const ConstantInt &Val, const SDLoc &DL,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, Opc, getVTList(EltVT), None);
  ID.AddPointer(Elt);
  ID.AddBoolean(isO);
  void *IP = nullptr;
  SDNode *N = nullptr;
...
SDValue SelectionDAG::getGlobalAddress(const GlobalValue *GV, const SDLoc &DL,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, Opc, getVTList(VT), None);
  ID.AddPointer(GV);
...
SDValue SelectionDAG::getTargetIndex(int Index, EVT VT, int64_t Offset,
                                     unsigned char TargetFlags) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::TargetIndex, getVTList(VT), None);
  ID.AddInteger(Index);
  ID.AddInteger(Offset);
  ID.AddInteger(TargetFlags);
  void *IP = nullptr;
...
SDValue SelectionDAG::getBasicBlock(MachineBasicBlock *MBB) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::BasicBlock, getVTList(MVT::Other), None);
  ID.AddPointer(MBB);
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, IP))
    return SDValue(E, 0);
...
SDValue SelectionDAG::getVectorShuffle(EVT VT, const SDLoc &dl, SDValue N1,
  ...
  FoldingSetNodeID ID;
  SDValue Ops[2] = { N1, N2 };
  AddNodeIDNode(TLI, ID, ISD::VECTOR_SHUFFLE, getVTList(VT), Ops);
  for (int i = 0; i != NElts; ++i)
...
SDValue SelectionDAG::getRegister(unsigned RegNo, EVT VT) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::Register, getVTList(VT), None);
  ID.AddInteger(RegNo);
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, IP))
    return SDValue(E, 0);
...
SDValue SelectionDAG::getRegisterMask(const uint32_t *RegMask) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::RegisterMask, getVTList(MVT::Untyped), None);
...
SDValue SelectionDAG::getLabelNode(unsigned Opcode, const SDLoc &dl,
  ...
  FoldingSetNodeID ID;
  SDValue Ops[] = { Root };
  AddNodeIDNode(TLI, ID, Opcode, getVTList(MVT::Other), Ops);
  ID.AddPointer(Label);
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, IP))
    return SDValue(E, 0);
...
SDValue SelectionDAG::getSrcValue(const Value *V) {
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::SRCVALUE, getVTList(MVT::Other), None);
  ID.AddPointer(V);
...
SDValue SelectionDAG::getMDNode(const MDNode *MD) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MDNODE_SDNODE, getVTList(MVT::Other), None);
  ID.AddPointer(MD);
...
SDValue SelectionDAG::getAddrSpaceCast(const SDLoc &dl, EVT VT, SDValue Ptr,
  ...
  SDValue Ops[] = {Ptr};
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::ADDRSPACECAST, getVTList(VT), Ops);
  ID.AddInteger(SrcAS);
...
SDValue SelectionDAG::getMetaData(const SDLoc &DL, const MDNode *MD) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MDNODE_SDNODE, getVTList(MVT::i32), None);
  ID.AddPointer(MD);
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, DL, IP))
    return SDValue(E, 0);
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT) {
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, Opcode, getVTList(VT), None);
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, DL, IP))
    return SDValue(E, 0);
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT,
  ...
  if (VT != MVT::Glue) { // Don't CSE flag producing nodes
    FoldingSetNodeID ID;
    AddNodeIDNode(TLI, ID, Opcode, VTs, Ops);
    void *IP = nullptr;
    if (SDNode *E = FindNodeOrInsertPos(ID, DL, IP)) {
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc &dl, EVT MemVT,
  ...
  AddNodeIDNode(TLI, ID, Opcode, VTList, Ops);
...
SDValue SelectionDAG::getLoad(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::LOAD, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
...
SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc &dl, SDValue Val,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::STORE, VTs, Ops);
...
SDValue SelectionDAG::getMaskedLoad(EVT VT, const SDLoc &dl, SDValue Chain,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MLOAD, VTs, Ops);
  ID.AddInteger(VT.getRawBits());
...
SDValue SelectionDAG::getMaskedStore(SDValue Chain, const SDLoc &dl,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MSTORE, VTs, Ops);
  ID.AddInteger(VT.getRawBits());
...
SDValue SelectionDAG::getMaskedGather(SDVTList VTs, EVT VT, const SDLoc &dl,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MGATHER, VTs, Ops);
  ID.AddInteger(VT.getRawBits());
...
SDValue SelectionDAG::getMaskedScatter(SDVTList VTs, EVT VT, const SDLoc &dl,
  ...
  FoldingSetNodeID ID;
  AddNodeIDNode(TLI, ID, ISD::MSCATTER, VTs, Ops);
  ID.AddInteger(VT.getRawBits());
...
SDNode *SelectionDAG::MorphNodeTo(SDNode *N, unsigned Opc,
  ...
  if (VTs.VTs[VTs.NumVTs-1] != MVT::Glue) {
    FoldingSetNodeID ID;
    AddNodeIDNode(TLI, N, ID, Opc, VTs, Ops);
