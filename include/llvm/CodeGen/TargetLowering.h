  ...
  public:
    ...
    ArgListEntry()
        : IsSExt(false), IsZExt(false), IsInReg(false), IsSRet(false),
          IsNest(false), IsByVal(false), IsJVMArgBuf(false) , IsInAlloca(false),
...
public:
  ...
  /// Target specific hook for avoiding DAG CSE based on the node opcode.
  virtual bool doNotCSE(unsigned Opcode) const { return false; }

  /// Preserve composite references, avoids disintegration of GEPs
  /// into Base + Offset computations instructions.
  virtual bool generateCompositeMacros() const { return false;}

  /// Generates DAG nodes of type ARRAY_REF/STRUCT_REF.
  virtual SDValue createCompositeMacros(SelectionDAGBuilder *SDB, const User &I,
                                        const SDLoc & dl) const {
    llvm_unreachable("Target does not implement composite macros");
  }

  virtual bool generateAllocationMacros() const {return false;}

  /// Generates DAG nodes for ALLOCA instruction.
  virtual SDValue createAllocationMacros(SelectionDAGBuilder *SDB, const User &I,
                                        const SDLoc & dl) const {
    llvm_unreachable("Target does not implement allocation macros");
  }

  virtual bool checkIfAbsorptionCandidate(SDValue V) const {
    return false;
  }

  /// Target specific node replicator counter for non CSEed nodes.
  /// Returns a positive integer for nodes which needs replication.
  virtual int getNodeReplicatorCounter(unsigned Opcode, SDNode * N = nullptr) {
     assert(doNotCSE(Opcode) && "Cannot return replicator counter nodes to be CSEed");
     return -1;
  }

  virtual void addNodeToReplicationMap(SDNode * N) {
     assert(doNotCSE(N->getOpcode()) && "Cannot add node to replicator map.");
  }

  /// Auxillary data structures to be removed after basic block processing. 
  virtual void clearAuxiliaryData() {}

  /// Do not perform implicit static allocation for alloca.
  virtual bool doNotPerformImplicitStaticAllocation() const {
    return false;
  }
 
  /// Skip post Lowering argument type validation, this is primarily
  /// used by JVM. 
  virtual bool skipArgumentTypeValidation(bool IsComposite) const {
    return false;
  }

  virtual bool skipDAGCombining(unsigned Opcode) const {
    return false;
  }
