...
template <typename T1, typename T2> bool compareTypes(T1 val1, T2 val2) {
  return std::is_same<T1, T2>();
}

/// \brief Returns option corrosponding to the Name
///  from the global command line parser (GlobalParser).
template <typename DataType> bool GetCommandLineOption(StringRef Name,DataType &retVal) {
  SubCommand *ActiveSubCommand = GetActiveSubCommand();
  if (!ActiveSubCommand)
    return false;

  auto OptItr = ActiveSubCommand->OptionsMap.find(Name);
  if (OptItr == ActiveSubCommand->OptionsMap.end())
    return false;

  //DataType DummyValue;
  opt<DataType> * optv = reinterpret_cast<opt<DataType>*>((*OptItr).getValue());
  auto &Value = optv->getValue();
#if 0
  // Redundant type-check.
  if (false == compareTypes(DummyValue, Value))
    report_fatal_error(
        "Type mismatch b/w Option's value type and requested value type");
#endif
