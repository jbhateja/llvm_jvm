...
protected:
  ...
  //===--- Options to emit in-memory ASM streaming ----------------------===//
  bool EnableInMemoryAsmEmit = false;

  /// Underlined buffer area of raw_string_ostream.
  std::string* AsmMemoryBuffer;
...
public:
  ...
  bool getEnableAsmInMemoryEmitting() const { return EnableInMemoryAsmEmit;}

  std::string& getAsmMemoryBuffer() const {
    assert(getEnableAsmInMemoryEmitting() && "ASM in-memory emitting is disabled");
    return *AsmMemoryBuffer; 
  }
