...
static int compileModule(char **argv, LLVMContext &Context) {
  ...
  bool InMemAsmStreaming = Target->getMCAsmInfo()->getEnableAsmInMemoryEmitting();
  // Figure out where we are going to send the output.
  std::unique_ptr<ToolOutputFile> Out = InMemAsmStreaming ? llvm::make_unique<ToolOutputFile>("-",0):
      GetOutputStream(TheTarget->getName(), TheTriple.getOS(), argv[0]);
  if (!Out) return 1;

  // Target specific validation for supplied command line options.
  Target->doCommandLineValidation();
