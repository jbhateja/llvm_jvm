 ...
 */
public class Main {
    public static boolean IsInLLVMToolChain = true;
    static {
       if (IsInLLVMToolChain)
         System.loadLibrary("LLVMJVMNativeUtils");
    }
...
"   or: jasmin -help";
    ...
    public native void llc_dump_classObj(byte [] classObj);

    public void assemble_string(String asmCode)
    {
        if (IsInLLVMToolChain) {
          System.out.println("assemble_string : Received AsmStream");
          System.out.println(asmCode);
        }
        String fname = "Dummy.jasm";
        File out_file = null;
        ClassFile classFile = new ClassFile();
        try {
            StringReader inp = new StringReader(asmCode);
            classFile.readJasmin(inp, fname, generate_linenum);
            ...
            System.out.println("Calling native function llc_dump_classObj..");
            llc_dump_classObj(outp.toByteArray());
